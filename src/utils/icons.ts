/**
 * Icon utility functions for rendering SVG icons in the grid
 * 
 * This module provides a centralized way to manage and retrieve SVG icons
 * used throughout the application. All icons are inline SVGs for better
...
/**
 * Collection of SVG icon definitions
 * 
 * Each icon is defined as an inline SVG string with:
 * - 16x16 viewBox for consistent sizing
 * - currentColor fill for CSS color inheritance
 * - Material Design inspired paths for visual consistency
 */
const iconDefinitions: Record<IconType, string> = {
  // Featured/premium items - filled star
  ...
  // Completed/verified items - checkmark
  ...
  // Urgent/warning items - triangle alert
  ...
  // Communication items - speech bubble
  ...
  // Support/help items - question mark in circle
  ...
  // Video/tutorial items - video camera
  ...
  // Individual user items - user profile
  ...
  // Community/group items - multiple users
...
/**
 * Retrieves the SVG string for a given icon type
 * 
 * @param iconName - The icon identifier from IconType
 * @returns SVG string for the requested icon, falls back to 'help' icon if not found
 * 
 * @example
 * ```typescript
 * const starIcon = getIconSvg('star');
 * // Returns: '<svg width="16" height="16"...star path...</svg>'
 * ```
 */
export const getIconSvg = (iconName: IconType): string => {
  return iconDefinitions[iconName] || iconDefinitions.help;
};

/**
 * Retrieves all available icon names
 * 
 * @returns Array of all available icon type strings
 * 
 * @example
 * ```typescript
 * const allIcons = getAvailableIcons();
 * // Returns: ['star', 'check', 'alert', ...]
 * ```
 */
export const getAvailableIcons = (): IconType[] => {
  return Object.keys(iconDefinitions) as IconType[];
};
 ...
 * @example
 * ```typescript
 * const isValid = isValidIcon('star'); // true
 * const isInvalid = isValidIcon('nonexistent'); // false
 * ```
 */
export const isValidIcon = (iconName: string): iconName is IconType => {
  return iconName in iconDefinitions;
};